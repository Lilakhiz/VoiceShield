#!/usr/bin/env python3
"""
Deepfake Detector Calibration Utility

This script evaluates the signal-heuristic deepfake detector against
labeled evaluation data (e.g., ASVspoof2019 LA).

Usage:
    python calibrate_deepfake.py --data-dir /path/to/asvspoof2019_LA --output results.json

Expected dataset structure (ASVspoof2019 LA):
    data_dir/
...
The protocol file format:
    ...
    where KEY is "bonafide" (genuine) or "spoof" (attack)
...
def evaluate_synthetic() -> dict:
    ...
    """
    from app.services.deepfake_detection import _f0_jitter, _harmonic_to_noise_ratio, _formant_band_flatness, _sigmoid
    ...
    for i in range(n_trials):
        ...
        result = detect_deepfake(live, SR)
        if result.available:
            y_true.append(0)
            y_scores.append(result.ai_generated_probability)
        
        # AI-like signal (zero jitter)
        ai = _ai_like_signal()
        result = detect_deepfake(ai, SR)
        if result.available:
            y_true.append(1)
            y_scores.append(result.ai_generated_probability)
    ...
    return {
        ...
        'num_samples': len(y_true),
        'note': 'Synthetic evaluation only - NOT calibrated on real ASVspoof data'
    }


def main():
    parser = argparse.ArgumentParser(
        description="Calibrate deepfake detector against labeled data"
    )
    parser.add_argument(
        '--data-dir', type=str, default=None,
        help='Path to ASVspoof2019 LA data directory'
    )
    parser.add_argument(
        '--protocol', type=str, default=None,
        help='Path to protocol file (default: ASVspoof2019_LA_asv_protocols/ASVspoof2019.LA.cm.eval.trl.txt under data-dir)'
    )
    parser.add_argument(
        '--audio-dir', type=str, default=None,
        help='Path to audio directory (default: ASVspoof2019_LA_eval under data-dir)'
    )
    parser.add_argument(
        '--output', type=str, default='calibration_results.json',
    ...
    if args.synthetic:
        ...
        if args.protocol:
            ...
            protocol_path = data_dir / 'ASVspoof2019_LA_asv_protocols' / 'ASVspoof2019.LA.cm.eval.trl.txt'
        
        if args.audio_dir:
            audio_dir = Path(args.audio_dir)
        else:
            audio_dir = data_dir / 'ASVspoof2019_LA_eval'
    ...
    print(f"\nRECOMMENDED: Set DEEPFAKE_THRESHOLD={metrics['optimal_threshold']:.4f} in environment")
    print("NOTE: This threshold is for the signal-heuristic fallback only.")
    print("      The pretrained model (if available) uses its own internal threshold.")
